...
Usage (from .venv): python scripts/landing/backfill_daily_bars.py

Writes via the Databricks Files API (not the FUSE-mount path
writer.write_landing_records assumes - that only works inside an
...
automatically (see ingestion/landing/client.py) - with 10 tickers this can
trigger a few waits, expect it to take a few minutes, not a hang.
"""
